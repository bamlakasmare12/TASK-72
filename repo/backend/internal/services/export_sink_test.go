...
func mockConfigService(configs map[string]string) *ConfigService {
	svc := &ConfigService{
		configs: make(map[string]models.Config),
		flags:   make(map[string]models.FeatureFlag),
	}
	for k, v := range configs {
		svc.configs[k] = models.Config{
			Key:       k,
			Value:     v,
			UpdatedAt: time.Now(),
		}
...
func TestExportSink_FileDrop_WritesCSVFile(t *testing.T) {
	...
	sink := NewExportSinkService(cfgSvc)
...
func TestLedgerToCSV_GeneratesValidCSV(t *testing.T) {
	...
	data, err := ledgerToCSV(entries)
	if err != nil {
		t.Fatalf("ledgerToCSV failed: %v", err)
	}
...
func TestSettlementsToCSV_GeneratesValidCSV(t *testing.T) {
	...
	data, err := settlementsToCSV(settlements)
	if err != nil {
		t.Fatalf("settlementsToCSV failed: %v", err)
	}
